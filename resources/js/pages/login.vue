...
const login = async () => {
  try {
    const resLogin = await $apiTest("auth/login", {
      method: "POST",
      body: JSON.stringify({
      ...
      onResponseError({ response }) {
        alert(`${response._data.message}! Tên đăng nhập hoặc mật khẩu không chính xác!`);
      },
    });

    const accessToken = resLogin.token;

    const userData = {
      id: 1,
      fullName: "New User",
      username: resLogin.username,
      avatar: resLogin.image,
    ...
    const subjectName = "Product Manager";
    const permissions = ["list", "add"];
        ...
        <VCardText>
          <VAlert color="primary" variant="tonal">
            <p class="text-caption mb-2 text-primary">
              Admin Username: <strong>emilys</strong> / Pass:
