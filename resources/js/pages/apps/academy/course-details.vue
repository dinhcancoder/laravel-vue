        ...
        <VCardItem title="UI/UX Basic Fundamentals" class="pb-6">
          ...
          <template #append>
            <div class="d-flex gap-4 align-center">
              ...
              <VIcon size="20" class="cursor-pointer" icon="ri-share-forward-line" />
              <VIcon size="20" class="cursor-pointer" icon="ri-bookmark-line" />
            </div>
          </template>
        </VCardItem>
        <VCardText>
                ...
                <div>
                  <VList class="card-list text-medium-emphasis">
                    <VListItem>
                      ...
                      <VListItemTitle
                        >Skill Level: {{ courseDetails?.skillLevel }}</VListItemTitle
                      >
                    </VListItem>
                    <VListItem>
                      <template #prepend>
                      ...
                      <VListItemTitle
                        >Students: {{ courseDetails?.totalStudents }}</VListItemTitle
                      >
                    </VListItem>
                    <VListItem>
                      <template #prepend>
                      ...
                      <VListItemTitle
                        >Languages: {{ courseDetails?.language }}</VListItemTitle
                      >
                    </VListItem>
                    <VListItem>
                      <template #prepend>
                        <VIcon icon="ri-closed-captioning-line" size="20" />
                      </template>
                      <VListItemTitle
                        >Captions: {{ courseDetails?.isCaptions }}</VListItemTitle
                      >
                    </VListItem>
                  </VList>
                </div>
                ...
                <div>
                  <VList class="card-list text-medium-emphasis">
                    <VListItem>
                      ...
                      <VListItemTitle
                        >Lectures: {{ courseDetails?.totalLectures }}</VListItemTitle
                      >
                    </VListItem>
                    <VListItem>
                      <template #prepend>
      ...
      <div class="course-content">
        <VExpansionPanels v-model="panelStatus" variant="accordion">
          <VExpansionPanel
            ...
            :expand-icon="
              $vuetify.locale.isRtl ? 'ri-arrow-left-s-line' : 'ri-arrow-right-s-line'
            "
            :value="index"
          >
            <template #title>
              <div>
                ...
                <div class="text-body-1">{{ section.status }} | {{ section.time }}</div>
              </div>
            </template>
            ...
            <template #text>
              <VList class="card-list">
                <VListItem v-for="(topic, id) in section.topics" :key="id" class="py-4">
                  <template #prepend>
                    <VCheckbox class="ps-3 me-3" :model-value="topic.isCompleted" />
                  </template>
                  <VListItemTitle>
                    <h6 class="text-h6">
                      {{ topic.title }}
